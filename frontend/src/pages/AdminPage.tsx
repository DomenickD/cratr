  ...
  const saveEntity = async () => {
    try {
      const payload = {
        ...
        // Internal name cannot be changed easily
      };
      ...
      if (selectedEntityId) {
          ...
          // Handle Fields (This is a simplified approach; in production, you'd diff them)
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-start">
        <div>
            <h1 className="text-4xl font-black text-slate-900 tracking-tight">System Studio</h1>
            <p className="text-slate-500 mt-1">Design your data models and visual workflows.</p>
        </div>
        
        <div className="flex bg-white p-1 rounded-2xl border border-slate-200 shadow-sm">
            <button 
                onClick={() => setActiveTab('entity')}
                className={`px-6 py-2 rounded-xl text-sm font-bold transition-all ${activeTab === 'entity' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:text-slate-900'}`}
            >
            ...
            <button 
                ...
                className={`px-6 py-2 rounded-xl text-sm font-bold transition-all disabled:opacity-30 ${activeTab === 'workflow' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:text-slate-900'}`}
            >
                <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-12 gap-8">
        {/* Sidebar: Existing Entities */}
        <div className="col-span-3 space-y-4">
            <div className="bg-white p-4 rounded-3xl border border-slate-200 shadow-sm">
                <div className="flex items-center gap-2 px-2 mb-4">
                    <Search size={16} className="text-slate-400" />
                    <input type="text" placeholder="Search models..." className="text-sm bg-transparent border-none focus:ring-0 w-full" />
                </div>
                <div className="space-y-1">
                    <button 
                        onClick={() => { setSelectedEntityId(null); setFields([]); setEntityName(''); setDisplayName(''); }}
                        className={`w-full text-left p-3 rounded-2xl text-sm font-bold flex items-center gap-3 transition-all ${!selectedEntityId ? 'bg-indigo-50 text-indigo-600' : 'text-slate-600 hover:bg-slate-50'}`}
                    >
                    ...
                    {entities?.map((e: any) => (
                        <button 
                            ...
                            className={`w-full text-left p-3 rounded-2xl text-sm font-bold flex items-center justify-between group transition-all ${selectedEntityId === e.id ? 'bg-indigo-600 text-white' : 'text-slate-600 hover:bg-slate-50'}`}
                        >
            ...
            {activeTab === 'entity' ? (
                <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-200 space-y-8">
                    <div className="flex justify-between items-center">
                        ...
                        <button 
                            onClick={saveEntity}
                            className="flex items-center space-x-2 bg-indigo-600 text-white px-8 py-3 rounded-2xl font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition-all active:scale-95"
                        >
                            <Save size={20} />
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <input 
                                ...
                                className="w-full rounded-2xl border-slate-200 p-4 font-bold text-slate-900 border"
                            />
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-between items-center px-1">
                            <h3 className="text-lg font-bold text-slate-900">Field Schema</h3>
                            <button 
                                onClick={addField}
                        ...
                        <div className="space-y-4">
                            {fields.map((field, fieldIndex) => (
                                <div key={fieldIndex} className="p-6 bg-slate-50 rounded-[1.5rem] border border-slate-100 space-y-4 relative group">
                                    <div className="flex items-end gap-4">
                                        <div className="flex-1 space-y-1">
                                            ...
                                            <input 
                                                ...
                                                className="w-full rounded-xl border-slate-200 p-2.5 text-sm font-bold border disabled:bg-slate-100"
                                            />
                                        </div>
                                        <div className="flex-1 space-y-1">
                                            ...
                                            <input 
                                                ...
                                                className="w-full rounded-xl border-slate-200 p-2.5 text-sm font-bold border"
                                            />
                                        </div>
                                        <div className="w-44 space-y-1">
                                            ...
                                            <select 
                                                ...
                                                className="w-full rounded-xl border-slate-200 p-2.5 text-sm font-bold border bg-white"
                                            >
                                                {FIELD_TYPES.map(t => (
                                        ...
                                        <button 
                                            ...
                                            className="h-[38px] w-[38px] flex items-center justify-center text-red-400 hover:bg-red-50 hover:text-red-600 rounded-xl transition-colors"
                                        >
                                            <Trash2 size={18} />
                                        </button>
                                    </div>
                                        ...
                                        <div className="ml-6 pl-6 border-l-2 border-indigo-200 space-y-3">
                                            <div className="flex justify-between items-center">
                                                ...
                                                <button 
                                                    ...
                                                    className="text-[10px] font-black bg-indigo-100 text-indigo-700 px-3 py-1 rounded-lg hover:bg-indigo-200 transition-colors"
                                                >
                                                    + Add Option
                                                </button>
                                            </div>
                                            <div className="grid grid-cols-3 gap-3">
                                                {field.configuration?.options?.map((option: string, optionIndex: number) => (
                                                    <div key={optionIndex} className="flex items-center gap-2 bg-white p-1 rounded-xl border border-slate-200 shadow-sm">
                                                        <input 
                                                            type="text"
                                                            value={option}
                                                            onChange={(e) => updateOption(fieldIndex, optionIndex, e.target.value)}
                                                            className="flex-1 text-xs border-none focus:ring-0 font-bold p-1"
                                                            placeholder="New option..."
                                                        />
                                                        <button onClick={() => removeOption(fieldIndex, optionIndex)} className="p-1 text-slate-300 hover:text-red-500">
                                                            <Trash2 size={14} />
                                                        </button>
                                                    </div>
                                                ))}
