...
const KanbanCard = ({ record }: { record: any }) => {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="bg-slate-700 p-4 rounded-xl border border-slate-600 shadow-sm hover:shadow-md transition-shadow group relative mb-3 cursor-default"
    >
...
export const KanbanColumn = ({ id, title, records }: { id: string, title: string, records: any[] }) => {
  ...
  return (
    <div className="bg-slate-800/70 rounded-2xl p-4 w-80 flex-shrink-0 flex flex-col h-full border border-slate-700">
      <div className="flex items-center justify-between mb-4 px-2">
        <h3 className="font-bold text-slate-300 uppercase tracking-wide text-sm flex items-center gap-2">
          {title}
