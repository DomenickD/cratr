...
class EntityDefinition(Base):
    ...
    fields = relationship("FieldDefinition", back_populates="entity_definition", cascade="all, delete-orphan")
...
class FieldDefinition(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    entity_definition_id = Column(Integer, ForeignKey("entity_definitions.id", ondelete="CASCADE"), nullable=False)
    name = Column(String, nullable=False) # e.g., "priority"
    display_name = Column(String, nullable=False)
    field_type = Column(String, nullable=False) # e.g., "text", "number", "date", "jsonb", "boolean"
